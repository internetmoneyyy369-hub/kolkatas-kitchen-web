...
export default function AddressesMain() {
  ...
  const [loadingAddressMutation, setLoadingAddressMutation] = useState(false);

  const mutate = async ({ variables }: any) => {
    if (!profile?._id) return;
    try {
      setLoadingAddressMutation(true);
      const newAddresses = (profile.addresses || []).filter((a: any) => a.id !== variables.id && a._id !== variables.id);
      ...
      setLoadingAddressMutation(false);
    }
  };

  // variables
  const addresses = profile?.addresses || [];
  const profileLoading = false; // Profile is from context now
  ...
  // UseEffects
  // Error handling moved to mutate function
