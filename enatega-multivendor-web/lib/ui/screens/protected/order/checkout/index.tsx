...
export default function OrderCheckoutScreen() {
  ...
  function couponCompleted({ coupon }: { coupon: any }) {
  ...
  const [loadingOrderMutation, setLoadingOrderMutation] = useState(false);
  const [couponLoading, setCouponLoading] = useState(false);

  useEffect(() => {
    const fetchTips = async () => {
      const { data, error } = await supabase.from('tipping').select('*').single();
      if (!error) setTipData(data);
    };
    fetchTips();
  }, []);
  ...
  const onApplyCoupon = async () => {
    try {
      ...
      if (error || !coupon) {
        couponCompleted({ coupon: { success: false } as any });
      } else {
        couponCompleted({ coupon: { ...coupon, success: true } as any });
      }
    } catch (err) {
  ...
  async function onPlaceOrder() {
    ...
    if (checkPaymentMethod(CURRENCY, paymentMethod)) {
      ...
      try {
        setLoadingOrderMutation(true);
        const { data: response, error: funcError } = await supabase.functions.invoke('place-order', {
          body: {
            restaurant: restaurantId,
            orderInput: items,
  ...
  // update function removed (Apollo cache no longer used)
