...
import TrackingOrderDetailsDummy from "@/lib/ui/screen-components/protected/order-tracking/components/tracking-order-details-dummy";
...
export default function OrderTrackingScreen({
  ...
  const hasUserReview = useMemo(() => {
    ...
    return reviewsData.reviewsByRestaurant.reviews.some(
      (review: IReview) =>
        review?.order?.user?.email === profile.email &&
            ...
            <PaddingContainer>
              ...
              <div className="flex flex-col md:flex-row md:items-start items-center justify-between gap-6 mb-8">
                ...
                <div className="md:ml-0 w-full md:w-auto md:flex-none">
                  <TrackingHelpCard />
                  {showChat &&
                    <ChatRider orderId={orderId} customerId={profile?._id || ''} />
                  }
                </div>
              </div>
