...
export default function PersonalInfoMain() {
  ...
  const { profile, loadingProfile: profileLoading } = useUser();
      ...
      <div className="p-6 w-full bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 ">
          <div>
            ...
            <TextComponent
              text={
                profile?.email_is_verified || profile?.emailIsVerified
                  ? t("Verified")
                  : t("Not Verified")
              }
              className={`text-sm font-medium ${profile?.email_is_verified || profile?.emailIsVerified ? "text-blue-500" : "text-red-500"}`}
            />
          </div>
          <div>
            ...
            <TextComponent
              text={
                profile?.phone_is_verified || profile?.phoneIsVerified
                  ? t("verified")
                  : t("not_verified")
              }
              className={`text-sm font-medium ${profile?.phone_is_verified || profile?.phoneIsVerified ? "text-blue-500" : "text-red-500"}`}
            />
          </div>
        </div>
        <UpdatePhoneModal
