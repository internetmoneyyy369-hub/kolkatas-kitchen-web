...
function Checkout(props) {
  ...
  const mutateCoupon = async({ variables }) => {
    try {
  ...
  const [dataTip, setDataTip] = useState({ tips: { tipVariations: [] } })

  useEffect(() => {
    const fetchTipping = async() => {
      try {
        const { data } = await supabase.from('tippings').select('*').eq('enabled', true)
        if (data) {
          setDataTip({ tips: { tipVariations: data.map(d => d.tip) } })
        }
      } finally {
        setLoadingTip(false)
      }
  ...
  const [mutateOrderLoading, setMutateOrderLoading] = useState(false)
  const mutateOrder = async({ variables }) => {
    try {
      setMutateOrderLoading(true)
      const { data, error } = await supabase.functions.invoke('place-order', {
        body: variables
      })
      if (error) throw error

      onCompleted({ placeOrder: data })
    } catch (error) {
      // Simulate GraphQL error shape for existing onError logic
      onError({
        message: error.message,
        graphQLErrors: error.message ? [{ message: error.message }] : []
      })
    } finally {
      setMutateOrderLoading(false)
    }
  }
  ...
  // Cache modifier logic skipped for Supabase
