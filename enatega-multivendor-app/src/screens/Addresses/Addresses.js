...
function Addresses() {
  ...
  const [loadingDeleteBulk, setLoadingDeleteBulk] = useState(false)
  ...
  const mutateBulkDelete = async({ variables }) => {
    try {
      setLoadingDeleteBulk(true)
      const { error } = await supabase
        .from('addresses')
        .delete()
        .in('id', variables.ids)
      ...
      setLoadingDeleteBulk(false)
    }
  }
  const { profile, refetchProfile } = useContext(UserContext)
  const networkStatus = loadingAddressMutation || loadingDeleteBulk ? 4 : 7
