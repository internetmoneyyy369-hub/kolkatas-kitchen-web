...
export default function GetHelpMain() {
  ...
  const handleSendMessage = useDebounceFunction(async () => {
    // Validate form data
    if (!reason) {
      showToast({
        type: "error",
    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          user_id: user?.id,
          order_id: reason === "order related" ? (orderId.length === 36 ? orderId : null) : null, // Ensure valid UUID if provided
          subject: finalTicketTitle,
          message: description,
          status: 'OPEN'
          // category: reason, // category is not in the schema anymore, but we can store it in subject or description if needed
        });
