...
export default function Cart({ onClose }: CartProps) {
  ...
  // Fetch related items from local restaurant data instead of Apollo
  const relatedItems = React.useMemo(() => {
    if (!data?.restaurant?.categories) return [];
    const allFoods = data.restaurant.categories.flatMap((cat: any) => cat.foods || []);
    // Filter out items already in cart and pick a few
    const cartIds = cart.map(item => item._id);
    return allFoods.filter((f: any) => !cartIds.includes(f._id)).slice(0, 3);
  }, [data, cart]);
