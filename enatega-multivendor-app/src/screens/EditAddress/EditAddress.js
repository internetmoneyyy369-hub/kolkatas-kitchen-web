  ...
  const mutate = async({ variables }) => {
    try {
      ...
      const { error } = await supabase
        .from('addresses')
        .update({
          label: variables.addressInput.label,
          delivery_address: variables.addressInput.deliveryAddress,
          details: variables.addressInput.details,
          location: {
            type: 'Point',
            coordinates: [parseFloat(variables.addressInput.longitude), parseFloat(variables.addressInput.latitude)]
          }
        })
        .eq('id', variables.addressInput._id)

      if (error) throw error

      // Since we don't get the full updated object back as easily in one go without .select(),
      // and the onCompleted expects the updated address:
      ...
      onCompleted({ editAddress: { ...updatedAddress, _id: updatedAddress.id } })
    } catch (err) {
      onError(err.message)
    } finally {
      setLoading(false)
