  ...
  const fetchReviews = async() => {
    try {
      ...
      const { data, error } = await supabase
        .from('reviews')
        .select('*, order:orders(user:profiles(name))')
        .eq('restaurant_id', restaurantId || restaurant?._id)

      if (error) throw error

      // Calculate aggregated ratings
      const total = data.length
      const ratings = total > 0 ? data.reduce((acc, r) => acc + r.rating, 0) / total : 0

      setReviewsData({
        reviewsByRestaurant: {
          reviews: data.map(r => ({ ...r, _id: r.id })),
          total,
        ...
        <View style={{ ...alignment.MTsmall }}>
          ...
          <View style={{ ...alignment.MBlarge }}>
            {sorted.map((review) => (
              <ReviewCard
                key={review._id}
                name={review.order?.user?.name || 'Anonymous'}
                description={review.description}
                rating={review.rating}
                date={calculateDaysAgo(review.created_at)}
                theme={currentTheme}
              />
            ))}
          </View>
