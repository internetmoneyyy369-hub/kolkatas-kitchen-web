...
function Profile(props) {
  ...
  const { profile, loadingProfile, refetchProfile } = useContext(UserContext)
  ...
  const { orderLoading, orderData, orderError } = useHomeRestaurants()
  const loading = loadingProfile
  const data = { userFavourite: profile?.userFavourite || [] }
