...
const CategoryPage = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    restaurantData.restaurant.categories.forEach((currentCategory) => {
      const seen = new Set()

      ;(currentCategory.foods || []).forEach((food) => {
        const title = food?.sub_category || food?.subCategory
        if (!title || seen.has(title)) return

        seen.add(title)
        derivedSubCategories.push({
          _id: `${currentCategory._id}-${title}`,
          id: `${currentCategory._id}-${title}`,
          title,
    ...
    setSubcategoriesData({ subCategories: derivedSubCategories })
