...
export default function UserAddressComponent(
  ...
  const [modifyingAddressLoading, setModifyingAddressLoading] = useState(false);

  const changeUserSelectedAddress = async ({ variables, onCompleted }: any) => {
    if (!profile?._id) return;
    try {
      setLoading(true);
      const newAddresses = (profile.addresses || []).map((a: any) => ({
        ...a,
        selected: a.id === variables.id || a._id === variables.id
      }));
      const { error } = await supabase.from('profiles').update({ addresses: newAddresses }).eq('id', profile._id);
  ...
  const mutate = async ({ variables }: any) => {
    if (!profile?._id) return;
    try {
      setModifyingAddressLoading(true);
      const { addressInput } = variables;
      let newAddresses = [...(profile.addresses || [])];

      if (addressInput._id) {
        // Edit
        newAddresses = newAddresses.map((a: any) => {
          if (a.id === addressInput._id || a._id === addressInput._id) {
            return {
              ...a,
              ...
              location: {
                type: 'Point',
                coordinates: [Number(addressInput.longitude), Number(addressInput.latitude)]
              }
            };
          }
          return a;
        ...
        // Unselect others if new one is selected
        newAddresses = newAddresses.map(a => ({ ...a, selected: false }));
      ...
      const result = {
        createAddress: !addressInput._id ? { addresses: newAddresses } : null,
        editAddress: addressInput._id ? { addresses: newAddresses } : null
      };
      onCompleted(result);
    } catch (err) {
      onError();
      console.error("Error mutating address:", err);
    } finally {
      setModifyingAddressLoading(false);
    }
  };
