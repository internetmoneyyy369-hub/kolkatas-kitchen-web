...
function Menu({ route, props }) {
  ...
  const [cuisinesData, setCuisinesData] = useState({ cuisines: [] })
  ...
  const refetchCuisines = async() => {
    try {
      let query = supabase.from('cuisines').select('*').eq('is_active', true)
      if (selectedType) {
        query = query.eq('shop_type', selectedType)
      }
      const { data: cuisineRows, error: cuisineError } = await query
      if (cuisineError) throw cuisineError

      setCuisinesData({
        cuisines: (cuisineRows || []).map((item) => ({
          ...item,
          _id: item.id,
          shopType: item.shop_type === 'grocery' ? 'Grocery' : 'Restaurant'
        }))
      })
      setCuisinesError(null)
    } catch (err) {
      setCuisinesError(err)
    }
  }
