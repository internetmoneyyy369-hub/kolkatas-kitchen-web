...
const SearchScreen = () => {
  ...
  const fetchOutlets = async() => {
    try {
      ...
      setRestaurants(data.map(r => ({ ...r, _id: r.id })))
    } catch (err) {
      console.log('Error fetching outlets in SearchScreen:', err)
    } finally {
      setLoading(false)
    }
  }
  ...
  // Combine all restaurants and remove duplicates is no longer needed as we fetch all
  // const restaurants = ... handled above in fetching all restaurants
