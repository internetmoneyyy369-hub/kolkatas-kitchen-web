...
export default function PhoneVerification({
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (otpResponse.data?.user && (!!formData?.phone || !!user?.phone)) {
        if (isRegistering) {
          ...
          const profileId = userData?.userId || otpResponse.data?.user?.id;
          if (profileId) {
          ...
          const profileId = user?.userId || authUser?.id;

          if (!profileId) {
            throw new Error("Unable to resolve profile ID for phone update.");
          }
          ...
          setOtp("");
          setPhoneOtp("");
          const emailIsVerified = !!authUser?.email_confirmed_at;

          if (!emailIsVerified && !user?.email) {
            handleChangePanel(5);
          } else if (!emailIsVerified && user?.email) {
            handleChangePanel(3);
          } else {
            handleChangePanel(0);
            setIsAuthModalVisible(false);
